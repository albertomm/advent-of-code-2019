...
def solve():
    ...
    masses = map(int, inputs)
    fuels = map(calculate_required_fuel, masses)
    result = sum(fuels)
    print(result)


def calculate_required_fuel(mass):
    result = max(0, (mass // 3) - 2)
    if result:
        result += calculate_required_fuel(result)
    return result


class TestDay2(unittest.TestCase):

    def test_calculate_required_fuel(self):
