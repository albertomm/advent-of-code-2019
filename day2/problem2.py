...
def solve():
    ...
    with open(input_path, 'r') as fd:
        initial_program = tuple(map(int, fd.read().split(',')))

    # Find the optimal inputs
    try:
        noun, verb = calculate_inputs(initial_program, 19690720)
...
def calculate_inputs(initial_program, expected):
    ...
    for noun, verb in possible_inputs:
        ...
        raise ValueError(
            "Cannot find valid inputs for expected result %s" % expected)
...
def run_program(memory):
    ...
    while True:
        ...
        # Find the opcode's function
        try:
            fn = FUNCTIONS[opcode]
        except KeyError:
            break

        # Get the operands positions
        pos1 = next(address)
        pos2 = next(address)
        pos3 = next(address)

        # Run the function
        memory[pos3] = fn(memory[pos1], memory[pos2])
...
class TestProblem(unittest.TestCase):
    ...
    def test_run_program(self):
        ...
        for case, expected in cases:
            ...
            self.assertSequenceEqual(case, expected)
